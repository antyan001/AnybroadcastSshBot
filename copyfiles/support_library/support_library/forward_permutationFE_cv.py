...
class ForwardPermutationFECV:
    ...
    def __init__(self, n=5, epsilon=0.005, metric='roc_auc', verbose=True,
        ...
        if folds is None:
            self.folds = KFold(n_splits=3, random_state=42)
        else:
            self.folds = KFold(n_splits=folds, random_state=42)
        self.n_jobs = n_jobs
    ...
    def fit_transform(self, X, y, model):
        ...
        for i, col in enumerate(self.permutation_importance_df['features']):
            ...
            if i > self.early_stopping_rounds and \
                                abs(current_metric - subsets[i-1]['score']) <= self.epsilon:
                return subsets, selected_features[:i]            
        ...
        return subsets, selected_features
