...
class Loader(object):
    ...
    def get_dataframe_clob(self, query, iskra='', password=''):
        """
        Return dataframe  for specified query.
    ...
    def _corpora_iter(self, df, num_partitions=12):
    ...
    def _push_balance_parallel(self, db, table_name, path, verbose, compress, isclobe, isuseclobdct, isallvarchar, njobs):

        print('Fetching dataframe from csv... ')
        with open(path) as f:
            if compress:
                reader = pd.read_csv(path, sep=self.sep,  encoding=self.encoding, compression='gzip', na_values='NaN', na_filter=False)
            else:
    ...
    def _push_balance(self, db, table_name, path, verbose, compress, isclobe, isuseclobdct, isallvarchar):
    ...
    def upload_csv(self, table_name, path='/data/data.csv', iskra='iskra4', password='',
                   parallel = 0, njobs=None, verbose=1, compress=1, isclobe = 0, isuseclobdct = 0, isallvarchar = 0):
        """
        ...
        try:
            if self.init_dsn:
                ...
                db = OracleDB('iskra', password, iskra)
            if not parallel:
                self._push_balance(db, table_name, path, verbose, compress, isclobe, isuseclobdct, isallvarchar)
            else:
                self._push_balance_parallel(db, table_name, path, verbose, compress, isclobe, isuseclobdct, isallvarchar, njobs)
